...
class DiametalMapCategoriesAndGroups

{
    ...
    public function mapGermanData($import) {
        foreach (json_decode($import->category_mapping) as $item) {
            var_dump($item);
            $parts = DiametalPartsDe::where($item->map_script, 'like', '%'.$item->validation_string.'%')->get();
    ...
    public function mapEnglishData($import) {
        ...
        foreach (json_decode($import->category_mapping) as $item) {
            $parts = DiametalPartsEn::where('material_group', $item->validation_string)->get();
            foreach ($parts as $part) {
                $part->update($this->categoryColumns($item, $import));;
            }
        }
