...
class DieboldMapCategoriesAndGroups

{
    ...
    public function mapGermanData($import) {
        foreach (json_decode($import->category_mapping) as $item) {
            $parts = DieboldPartsDe::where('part_name', 'like', '%'.$item->validation_string.'%')->get();
    ...
    public function mapEnglishData($import) {
        foreach (json_decode($import->category_mapping) as $item) {
            $partsDeIds = DieboldPartsDe::where('part_name', 'like', '%'.$item->validation_string.'%')->pluck('part_id');
            $parts = DieboldPartsEn::whereIn('part_id', $partsDeIds)->get();
            foreach ($parts as $part) {
                $part->update($this->categoryColumns($item, $import));;
            }
        }
