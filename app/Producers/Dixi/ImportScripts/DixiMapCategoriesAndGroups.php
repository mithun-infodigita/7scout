...
class DixiMapCategoriesAndGroups

{
    ...
    public function mapGermanData($import) {
        foreach (json_decode($import->category_mapping) as $item) {
            $parts = DixiPartsDe::where('material_group', $item->validation_string)->get();
    ...
    public function mapEnglishData($import) {
        ...
        foreach (json_decode($import->category_mapping) as $item) {
            $parts = DixiPartsEn::where('material_group', $item->validation_string)->get();
            foreach ($parts as $part) {
                $part->update($this->categoryColumns($item, $import));;
            }
        }
    ...
    public function mapFrenchData($import) {
        foreach (json_decode($import->category_mapping) as $item) {
            $parts = DixiPartsFr::where('material_group', $item->validation_string)->get();
            foreach ($parts as $part) {
                $part->update($this->categoryColumns($item, $import));;
            }
        }
    ...
    public function mapItalianData($import) {
        foreach (json_decode($import->category_mapping) as $item) {
            $parts = DixiPartsIt::where('material_group', $item->validation_string)->get();
            foreach ($parts as $part) {
                $part->update($this->categoryColumns($item, $import));;
            }
        }
